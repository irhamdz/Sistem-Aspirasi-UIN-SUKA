...
for(var i=0; i<data_item.length; i++)
    	{
    		$('#'+data_item[i]).show();
					...
					$.ajax(
					{
						url 	: "<?php echo base_url('adminpmb/form_control/cek_tesis'); ?>",
						type	: "POST",            
						data: "id="+id,
...
function simpan_tesis()
	{
					...
					$.ajax(
					{
						url 	: "<?php echo base_url('adminpmb/form_control/insert_tesis'); ?>",
						type	: "POST",            
						data: $("#data_proposal_tesis").serialize(),
						success: function(x)
						{
							
							$('#slide-form').load("<?php echo base_url('adminpmb/form_control/data_proposal_tesis/"+id+"'); ?>");
...
function hapus_tesis()
	{
			$.ajax(
					{
						url 	: "<?php echo base_url('adminpmb/form_control/hapus_tesis'); ?>",
						type	: "POST",            
						data    : "id="+id,
						success: function(z)
						{
