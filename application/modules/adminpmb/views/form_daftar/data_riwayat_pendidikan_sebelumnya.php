...
var id=$('#nomor').attr('value');
	function simpan_pend()
	{
		...
		if(cek_ada < 1 && sekolah.length > 0 && kode_sekolah.length >0)
		{
			...
			$.ajax(
					{
						url 	: "<?php echo base_url('adminpmb/form_control/insert_riwayat_pendidikan'); ?>",
						type	: "POST",            
						data: $("#data_riwayat_pendidikan_sebelumnya").serialize(),
						success: function(x)
						{
							
							$('#slide-form').load("<?php echo base_url('adminpmb/form_control/data_riwayat_pendidikan_sebelumnya/"+id+"/"+jalur+"'); ?>");
	...
	function hapus_pend()
	{
		...
		document.getElementById('msg').scrollIntoView();
			$.ajax(
					{
						url 	: "<?php echo base_url('adminpmb/form_control/hapus_riwayat_pendidikan'); ?>",
						type	: "POST",            
						data    : "id="+id,
						success: function(z)
						{
...
function sekolah_cari(inputString,param_lokasi,param_lokasi_balik,param_lokasi_tampil){
	if(inputString.length == 0) {
		...
		type: "POST",
		cache:false,
		url: "<?php echo base_url('adminpmb/form_control/cari_sekolah') ?>",
