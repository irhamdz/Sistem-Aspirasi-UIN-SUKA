...
var id=$('#nomor').attr('value');
	function simpan_karya_tulis()
	{
			...
			if(judul.length > 5)
			{
				if(filenya.length > 20)
				{
					$.ajax(
					{
						url 	: "<?php echo base_url('adminpmb/form_control/insert_karya_tulis'); ?>",
						type	: "POST",            
						data: $("#data_karya_tulis").serialize(),
						success: function(x)
						{
						
							$('#slide-form').load("<?php echo base_url('adminpmb/form_control/data_karya_tulis/"+id+"'); ?>");
	...
	function hapus_karya_tulis(x)
	{
			...
			$.ajax(
					{
						url 	: "<?php echo base_url('adminpmb/form_control/hapus_karya_tulis'); ?>",
						type	: "POST",            
						data    : "id="+x,
						success: function(z)
						{
