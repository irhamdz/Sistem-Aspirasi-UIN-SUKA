...
<div>
<h3 style="margin-bottom:10px;">PENILAIAN DOKUMEN MAHASISWA</h3>
	<table class="table table-hover">
		...
		<tr>
		<td>Jadwal Ujian</td>
			<td id='td-jadwal'><select name='kode_jadwal' style='width:300px;' id='jadwal' class='form-control input-md' onchange='jadwal_porto=this.value'>";
				<option value=''>Pilih Jadwal Ujian</option>
		</tr>
		<tr>
		<td></td>
			<td>
				<button class="btn btn-inverse btn-uin btn-small" type="button" onclick="doc_jalur()"> Cari</button>
			</td>
		</tr>
...
function doc_jalur () {
	$("#table-doc-mhs").html('<br /><center><img src="http://akademik.uin-suka.ac.id/asset/img/loading.gif"><br />Harap menunggu</center>');
...
var kode_penawaran=kode_jalur+gelombang+tahun;
	...
	$.ajax({
		url: "<?php echo base_url('adminpmb/input_data_c/cari_nilai_dokumen'); ?>",
		type: "POST",
		data: "kode_penawaran="+kode_penawaran+"&tahun="+tahun+"&kode_jadwal="+jadwal_porto,
		success: function(doc){
			$('#table-doc-mhs').html(doc);
		}
	});
...
function jadwal_lihat(jl)
{
	var kode_jalur=$('#pena').val();
	var gelombang=$('#gelombang').val();
	var kode_penawaran=kode_jalur+gelombang+jl;//jalur+gelombang+tahun
	$.ajax({
