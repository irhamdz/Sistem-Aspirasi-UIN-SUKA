	...
	<thead>
		...
		<th>Tempat Bayar</th>
		<th style="width:100px; background-color:#FFC;">Nominal Bayar</th>
		</tr>
	</thead>
	<tbody>
	<?php $i = 0; $totalb = 0; foreach ($data['bayar_sort'] as $d1): 
		...
		switch($data['view_bayar']){
			case 'mhs': case 'wali': 
				$url_detil = str_replace('%LINK%',t1_encode($d1),$data['url_d1']); 
				#$url_detil = $data['url_link'].'/bayar-dtl-'.t1_encode($d1).'.html';
			break;
			case 'dosen': 
				$url_detil = str_replace('%LINK%',t1_encode($data['mahasiswa'][0]['NIM'].'#'.$d1),$data['url_d1']); 
				#$url_detil = 'dosen/bayar-mhsdt-'.t1_encode($data['mahasiswa'][0]['NIM'].'#'.$d1).'.html';
			break;
		...
		<td class=""><?php echo anchor($url_detil,$d['NM_KOMPONEN'],'class="link" title="lihat detil pembayaran '.$d['NM_KOMPONEN'].' tanggal '.$d['TGL_TRANSAKSI'].'"'); ?></td>
		<td class="tac"><?php 
		...
		?></td>
		<td class="tac"><?php echo $d['TGL_TRANSAKSI'].' WIB'; ?></td>
		<td class="tac"><?php echo $d['NM_BANK']; ?></td>
		...
		<tr>
		<td colspan="6"  style="border-top:1px solid #999;"><strong>Jumlah Seluruh Pembayaran</strong></td>
	...
	echo form_close();
	?>
	<?php else: echo '<br><h1>Tidak ada data pembayaran untuk ditampilkan!</h1>'; ?>	
	<?php endif; ?>
