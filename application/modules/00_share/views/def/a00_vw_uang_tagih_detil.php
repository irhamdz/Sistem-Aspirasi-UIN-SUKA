	...
	'','Waktu Bayar','','Nominal Tagihan','Kode Tagihan',
	'Rincian','Status Tagihan','','');
	
	function trans_status_tagih($kode){
		switch($kode){
			case 'L': return 'LUNAS'; break;
			case 'H': return 'HANGUS'; break;
			case 'B': return 'BELUM LUNAS'; break;
			case 'P': return 'PENDING'; break;
			default: return $kode; break;
		}		
	}
	...
	$cat = trim(strtolower($d10[1]));
			switch($cat){
				...
				case 'spp': $arr_k1 = array(0,5,7,2,6,4); break; #array(1,2,3,4,5,7,8,9,10,11); break;
				case 'spv': $arr_k1 = array(0,5,7,2,6,4); break; #array(1,2,3,4,5,7,8,9,10,11); break;
	...
	<h2>Detil Tagihan Mahasiswa</h2>
	<table class="table table-bordered table-hover">
	<?php foreach($arr_k1 as $ar1): ?>
	<tr style="<?php 
		if (($ar1 == 6) || ($ar1 == 4) || ($ar1 == 9)){ echo 'background-color:#FFC;'; }
	?>">
	<th style="text-align:left; width:225px;"><?php echo $arr_kolom1[$ar1]; ?></th>
	<td>
	<?php
		switch($ar1){
			case 0: 
			...
			case 2:
				...
				$tgl = date_trans_foracle($d['TGL_STARTED'].'', 1, '1 231 111', ' ');
				echo substr($tgl,0,strlen($tgl)-9).', '.substr($tgl,strlen($tgl)-8,8).' WIB';
				$tgl = date_trans_foracle($d['TGL_EXPIRED'].'', 1, '1 231 111', ' ');
				echo ' s/d '.substr($tgl,0,strlen($tgl)-9).', '.substr($tgl,strlen($tgl)-8,8).' WIB';
			break;
			case 3:
				
			break;
			case 6:
				echo $d['QTY'].' × Rp'.number_format((int)$d['TARIF'],0,'.','.');
			break;
			case 4: echo 'Rp'.number_format((int)$d['TOTAL_BAYAR'],0,'.','.'); break;
			case 5: echo $d['ID_TAGIHAN']; break;
			case 7: echo '<strong>'.trans_status_tagih($d['STATUS_BAYAR']).'</strong>'; break;
		}
	?>
	</td>
	</tr>
