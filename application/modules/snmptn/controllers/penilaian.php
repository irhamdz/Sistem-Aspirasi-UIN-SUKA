...
/*== PEHITUNGAN PERINGKAT SEKOLAH ==*/
	function nilai_peringkat_sekolah(){
		...
		//print_r($arr_na);
		if($_POST == null){
			$arr_na=$this->webserv->snmptn('penilaian/get_peringkat_sekolah',array('KD_PRODI'=>$kd_prodi,'PTN'=>$ptn));
			$data['na']=$arr_na;
			$data['content']='snmptn/penilaian/nilai_peringkat_sekolah';
			$this->load->view('admin/header',$data);
			$this->load->view('admin/content');
		}else{
			$rs=$this->lib_yudisium->api_yudisium('snmptn/yudisium/set_peringkat_sekolah',array('KD_PRODI'=>$kd_prodi,'PTN'=>$ptn));
			//print_r($rs);
			$this->session->set_flashdata('message',array("error","Data nilai peringkat siswa berhasil disimpan."));
			redirect('snmptn/penilaian/nilai_peringkat_sekolah');
		}	
...
/*== PEHITUNGAN NILAI AKADEMIK ==*/	
	function nilai_akademik(){
		...
		if($_POST == null){
			...
			$data['siswa']=$this->webserv->snmptn('penilaian/get_nilai_akademik',$postdata);
			//print_r($data['siswa']);die();
			$bobot=$this->webserv->snmptn('penilaian/get_bobot_mata_pelajaran',array('KD_PRODI'=>$kd_prodi));
			$arr_bobot=array();
			if(!empty($bobot)){
				foreach($bobot as $p){
			...
			$data['content']='snmptn/penilaian/nilai_akademik';
			$this->load->view('admin/header',$data);
			$this->load->view('admin/content');
			...
			$bobot=$this->webserv->snmptn('penilaian/set_bobot_mata_pelajaran',array('KD_PRODI'=>$kd_prodi,'NILAI'=>$nilai));
			#print_r($bobot);die();
			$rs=$this->webserv->snmptn('penilaian/set_nilai_akademik',array('KD_PRODI'=>$kd_prodi,'PTN'=>$ptn));
			#if($rs){
				$this->session->set_flashdata('message',array("error","Data nilai mata pelajaran berhasil disimpan."));
				redirect('snmptn/penilaian/nilai_akademik');
			#}	
		}	
	} 
...
/*== PEHITUNGAN NILAI PRESTASI ==*/	
	function nilai_prestasi(){	
		...
		if($_POST == null){
			...
			$data['siswa']=$siswa=$this->webserv->snmptn('penilaian/get_siswa_prodi',$postdata);
			$arr_siswa=array();		
			if(!empty($siswa)){			
				foreach($siswa as $s){
			...
			$data['content']='snmptn/penilaian/nilai_prestasi';
			$this->load->view('admin/header',$data);
			$this->load->view('admin/content');
		//	$this->load->view('page/footer');
			...
			$data=$this->webserv->snmptn('penilaian/set_nilai_prestasi',$postdata);
			if($data){
				$this->session->set_flashdata('message',array("error","Data nilai prestasi non akademik berhasil disimpan."));
				redirect('snmptn/penilaian/nilai_prestasi');
			}
		}	
	} 

/*== PEHITUNGAN NILAI RIWAYAT SNMPTN ==*/
	function nilai_riwayat_snmptn(){
		if($_POST == null){			
			$data['nilai']=$this->lib_yudisium->api_yudisium('snmptn/yudisium/get_riwayat_snmptn');
			$data['content']='snmptn/penilaian/nilai_riwayat_snmptn';
			$this->load->view('s00_vw_all', $data);
		}else{			   
			$data=$this->lib_yudisium->api_yudisium('snmptn/yudisium/set_riwayat_snmptn');
			print_r($data);die();
			$this->session->set_flashdata('message',array("error","Data nilai riwayat SNMPTN disimpan."));
			//redirect('snmptn/penilaian/nilai_riwayat_snmptn');
		}	
...
/*== NILAI YUDISIUM ==*/	
	function nilai_yudisium(){
		...
		$dp=$this->lib_yudisium->api_yudisium('snmptn/yudisium/get_prodi');
		$data['prodi']=$dp;
		$kd_prodi=$this->session->userdata('kd_prodi');
		$data['kode_prodi']=$kd_prodi;
		$p=$this->lib_yudisium->api_yudisium('snmptn/pembobotan/get_putaran');
		$data['pilihan']=$pilihan=$p->pilihan_aktif;
		$ptn=$p->pt_aktif;	
		...
		if($_POST == null){
				...
				if(isset($kd_prodi)and $kd_prodi!=null){
					...
					$data['kode_prodi']=$kd_prodi;
					/*========================*/
				/* 	$prestasi=$this->lib_yudisium->api_yudisium('snmptn/yudisium/get_prestasi');
					$arr_prestasi=array();
					foreach($prestasi as $p){
						if(!isset($arr_prestasi[$p->NOMOR_PENDAFTARAN])) {
							$arr_prestasi[$p->NOMOR_PENDAFTARAN] = array();
			...
			$data['content']='snmptn/penilaian/nilai_yudisium';
			$this->load->view('admin/header',$data);
			$this->load->view('admin/content');
		}
	} 
	...
	function nilai_yudisium2(){
		...
		$ptn=$p->PT_AKTIF;	
				...
				$bobot=$this->lib_yudisium->api_yudisium('snmptn/yudisium/get_pembobotan');
				foreach($bobot as $b){
					$bobot2[$b->KODE_NILAI]=$b->BOBOT;
				}
				$data['b']=$bobot2;
		if($_POST == null){
				...
				$data['siswa']=$this->lib_yudisium->api_yudisium('snmptn/yudisium/get_nilai_yudisium',$postdata);
				}
			$data['content']='snmptn/penilaian/nilai_yudisium';
				$this->load->view('s00_vw_all', $data);
		}
