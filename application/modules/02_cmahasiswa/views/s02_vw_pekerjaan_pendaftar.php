				...
				switch($status_pekerjaan){
					...
					case 3:  
							foreach($master_pekerjaan as $value){ 
								if($value->PMB_ID_PEKERJAAN==$status_pekerjaan){
									echo "<option value=".$value->PMB_ID_PEKERJAAN." selected>".$value->PMB_NAMA_PEKERJAAN."</option>";
								}else{
					...
					default : 
						...
						echo "<option value='$status_pekerjaan' selected>$status_pekerjaan</option>";
					break;
				}
				  ?>
				 <option value='pekerjaan_lain'>Lainnya</option></select></div> <br /><br />
				<div class="col-xs-7"><input type="text" class="form-control input-sm" id="pmb1_status_pekerjaan" class="form-control input-sm" name="pekerjaan_lain" value="" style="display: none;"></div>  *)</td>
		</tr>
		<tr>
			<td >Alamat Kantor</td>
			<td><div class="col-xs-7"><textarea name='pmb1_alamat_kantor' class="form-control input-sm" ><?php echo $pekerjaan[0]->PMB_ALAMAT_KANTOR; ?></textarea></div>
			<div class="col-xs-12"><div class="reg-info">Silahkan diisi dengan tanda " - " jika tidak ada.</div></div></td>
		</tr>
		<tr>
			<td >No. Telp./Fax</td>
			<td><div class="col-xs-7"><input type='text' name='pmb1_telp_fax' class="form-control input-sm" value="<?php echo $pekerjaan[0]->PMB_TELP_FAX_KANTOR; ?>" /></div>
			<div class="col-xs-12"><div class="reg-info">Silahkan diisi dengan tanda " - " jika tidak ada.</div></div></td>
		</tr>
		<tr>
			<td >Rencana Biaya Studi</td>
			<td><div class="col-xs-7">
			<select name='pmb1_rencana_biaya' id='biaya' class="form-control input-sm">
					<option value=''>Pilih</option>
				<?php 
				...
				switch($rencana_biaya){
					...
					case 4:  
							foreach($master_biaya as $value){ 
								if($value->PMB_ID_RENCANA_BIAYA==$rencana_biaya){
									echo "<option value=".$value->PMB_ID_RENCANA_BIAYA." selected>".$value->PMB_NAMA_RENCANA_BIAYA."</option>";
								}else{
					...
					default : 
						...
						echo "<option value='$rencana_biaya' selected>$rencana_biaya</option>";
					break;
				}
				  ?>
				 <option value='biaya_lain'>Lainnya</option></select></div> <br /><br />
				<div class="col-xs-7"><input type="text" class="form-control input-sm" id="pmb1_rencana_biaya" name="biaya_lain" value="" style="display: none;"></div> 	
				*)
			</td>
		</tr>
		<tr>
			...
			<td align='right'><input type='hidden' name='step' value='update_pekerjaan' /><?php echo form_submit('pmb1_pekerjaan_submit', 'Selanjutnya >>', 'class="btn-uin btn btn-inverse btn btn-small"'); ?></td>
		</tr>
	</tbody>
	</table>
	<?php echo form_close(); 
