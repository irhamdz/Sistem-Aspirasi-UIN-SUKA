...
class lib_reg_fungsi {
	function _0295_jebakan($kode = ''){
		$CI=&get_instance();
		$v_nim = $CI->session->userdata('id_user');
		$arra = array($v_nim, 'DPM-'.$kode);
		$CI->s00_lib_api->get_api_json(URL_API_SIA.'sia_sistem/data_procedure', 'POST', 
			array('api_kode'=>101001, 'api_subkode' => 8, 'api_datapost' => $arra));
	}
	function cek_awal(){
	   	error_reporting(0);
		$CI=&get_instance();
	...
	function get_data_file_siswa($NO_TEST,$kolom){
		$API_URL="http://service.uin-suka.ac.id/servadmisi/index.php/praregistrasi/d_mahasiswa/get_uploaded_data?no_test=$NO_TEST&kolom=$kolom";
		$hasil=$this->api_admisi($API_URL,array(),'GET');
		return json_decode($hasil,true);
	}
    ...
    function cek_step($kd_prodi){
    	$CI=&get_instance();
    	/*
    	$max = 15; /////////////////
    	$kd_ta = $CI->session->userdata('kd_ta');
    	$kd_smt = $CI->session->userdata('kd_smt');
    	$aksi = $CI->s00_lib_api->get_api_json(URL_API_SIA.'sia_sistem/data_search', 'POST', 
		array('api_kode'=>200001, 'api_subkode' => 18, 'api_search' => array($kd_ta,$kd_smt,$kd_prodi,0)));
		foreach($aksi as $k => $v){
			$stepx = (int)str_replace("HALAMAN","",$v['SY_ISI']);
			if($stepx > $max){
				continue;
			}
			$step[] = $stepx;
	...
	function cek_kelengkapan($nim){
		...
		*/
		...
		Untuk data darurat
		*/
		$data_darurat = $this->data_kontak_darurat_ambil($nim);
		if(count($data_darurat)<1){
			$kurang_hal15=1;
			$err15=$err15."<li>Data kontak darurat belum diisi.</li>";
			$kurang['halaman15']=$err15;
		}
		return $kurang;
	}
	...
	//DATA DARURAT ISI
	function data_kontak_darurat_isi($ARR_DATA){
        $CI=&get_instance();
		$isi=$CI->s00_lib_api->get_api_json(URL_API_SIA.'sia_mahasiswa/data_procedure', 'POST', 
						array('api_kode'=>27009, 'api_subkode' =>1, 'api_datapost' => $ARR_DATA));
		return $isi;
    }
    function data_kontak_darurat_ambil($nim){
        $CI=&get_instance();
		$isi=$CI->s00_lib_api->get_api_json(URL_API_SIA.'sia_mahasiswa/data_search', 'POST', 
						array('api_kode'=>27009, 'api_subkode' =>1, 'api_search' => array($nim)));
		return $isi;
    }
    //DATA KEUANGAN
