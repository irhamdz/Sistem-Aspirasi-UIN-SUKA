...
<div  class="article-content">
			...
			<div class="isi">
			<table width="100%">
					<tr><td>Hari</td><td>:</td><td><?php echo tgl_format($d->tgl_mulai,2,4); if($d->tgl_selesai!=$d->tgl_mulai and $d->tgl_selesai!="0000/00/00") echo " s.d. ".tgl_format($d->tgl_selesai,2,4);?></td></tr>
					<tr><td>Tanggal</td><td>:</td><td><?php echo tgl_format($d->tgl_mulai,2,3); if($d->tgl_selesai!=$d->tgl_mulai and $d->tgl_selesai!="00/00/0000") echo " s.d. ".tgl_format($d->tgl_selesai,2,3); ?></td></tr>
					...
					<tr><td valign="top">Deskripsi</td><td valign="top"> : </td><td><?php if($d->deskripsi ==null){ echo "-"; } ?></td></tr>
					<?php
					if($d->deskripsi !=null){ 
						echo"<tr><td colspan='3'>".html_entity_decode($d->deskripsi)."</td></tr>"; 
					}
					?>
					<?php
					if($d->lampiran !=null){ ?>
					<tr><td colspan='3'>
					<?php if($ext=='jpg' or $ext=='png'){?>
					<img src="<?php echo base_url().'media/agenda/'.$d->lampiran ?>" />
					<?php }else { ?>
				 <iframe width="100%" height="500" src="<?php echo base_url().'media/agenda/'.$d->lampiran ?>" ></iframe> 
				 <?php } ?>
				 </td></tr>
				<?php } ?>
				</table>
			...
			<div class="related-article left">
				...
				foreach($rec as $r){?>
					...
					<span class="tgl-post"><?php echo nama_hari($r->tgl_posting).', '.tanggal_indonesia($r->tgl_posting).' '.$r->tgl_posting ?> WIB</span>
