<h2>Riwayat Jadwal ICT Peserta</h2>
<?php
	$crumbs = array(
				array('Training & Sertifikasi' => 'training/admin'),
				array('ICT' => '#'),
				array('Riwayat Jadwal ICT Peserta' =>'operator/historipeserta'),
			);
	$this->it00_lib_output->output_crumbs($crumbs);

?>
<div id = "info" class="bs-callout bs-callout-info">Silakan masukan nomor peserta untuk melihat jadwal ICT yang diambil.</div>
<div id="input_nim" style="margin-bottom:20px;">
	<form name="fnim"  id="fnim" method="post" action="">
	<input type="hidden" name="op" value="search">
		<table>
			<tr>
				<td class="input-medium">Nomor Peserta</td>
				<td><input id="nim" class="form-control" type="text" value="<?php if(isset($nim)){echo $nim;} else{ echo '';}?>" name="nim"> </td>
			</tr>
			<tr>
				<td></td>
				<td><button class="btn btn-small btn-inverse"><i class="icon-white icon-search"></i> Cari peserta</button></td>
			</tr>
		</table>
	</form>
</div>
