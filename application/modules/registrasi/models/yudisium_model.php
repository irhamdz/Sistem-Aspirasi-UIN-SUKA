...
class Yudisium_model extends CI_Model {
	...
	function rekap_nilai($ptn=0){
			...
			$nilai=$this->db->query("SELECT PROGRAM_STUDI,URUTAN_PROGRAM_STUDI,COUNT(SELISIH_KUADRAT), SUM(SELISIH_KUADRAT) ,
				...
				( (NILAI_AKREDITASI*".$bobot['NILAI_MAPEL'].")+
						(NILAI_MAPEL*".$bobot['NILAI_MAPEL'].")+
						(NILAI_RANKING*".$bobot['NILAI_PERINGKAT_SISWA'].")+
						(NILAI_PRESTASI*10*".$bobot['NILAI_PRESTASI'].")+
						(NILAI_PERINGKAT_SEKOLAH*".$bobot['NILAI_PERINGKAT_SEKOLAH'].")+
						(NILAI_SEBARAN_WILAYAH*".$bobot['NILAI_SEBARAN_WILAYAH'].")+
						(NILAI_UN*".$bobot['NILAI_UN']."))/100 TOTAL_NILAI
				FROM NILAI_YUDISIUM
				WHERE URUTAN_PTN='".$ptn."'
				AND  NILAI_YUDISIUM.DITERIMA_PTN_LAIN='0'
				) TN
				LEFT JOIN (
				...
				AVG(( (NILAI_AKREDITASI*".$bobot['NILAI_MAPEL'].")+
						(NILAI_MAPEL*".$bobot['NILAI_MAPEL'].")+
						(NILAI_RANKING*".$bobot['NILAI_PERINGKAT_SISWA'].")+
						(NILAI_PRESTASI*10*".$bobot['NILAI_PRESTASI'].")+
						(NILAI_PERINGKAT_SEKOLAH*".$bobot['NILAI_PERINGKAT_SEKOLAH'].")+
						(NILAI_SEBARAN_WILAYAH*".$bobot['NILAI_SEBARAN_WILAYAH'].")+
						(NILAI_UN*".$bobot['NILAI_UN']."))/100) RERATA 
				FROM NILAI_YUDISIUM
				WHERE URUTAN_PTN='".$ptn."'
				AND  NILAI_YUDISIUM.DITERIMA_PTN_LAIN='0'
	...
	function rekap_diterima($ptn=0){
			$diterima=$this->db->query("SELECT FAKULTAS.NAMA_FAKULTAS,PROGRAM_STUDI.PROGRAM_STUDI,NILAI_YUDISIUM.URUTAN_PROGRAM_STUDI,COUNT(NILAI_YUDISIUM.NOMOR_PENDAFTARAN) JUMLAH FROM NILAI_YUDISIUM
									LEFT JOIN PROGRAM_STUDI ON PROGRAM_STUDI.KODE_PROGRAM_STUDI = NILAI_YUDISIUM.KODE_PROGRAM_STUDI
									LEFT JOIN FAKULTAS ON PROGRAM_STUDI.KODE_FAKULTAS = FAKULTAS.KODE_FAKULTAS
			...
			foreach($diterima as $p){
				...
				if(!isset($arr_diterima[$p['NAMA_FAKULTAS']][$p['PROGRAM_STUDI']])) {
					$arr_diterima[$p['NAMA_FAKULTAS']][$p['PROGRAM_STUDI']] = array();
				}
				 $arr_diterima[$p['NAMA_FAKULTAS']][$p['PROGRAM_STUDI']][] = $p;
			}			
			return $arr_diterima;
	}
