...
for(var i=0; i<data_item.length; i++)
    	{
    		$('#'+data_item[i]).show();
					...
					$.ajax(
					{
						url 	: "<?php echo base_url('pendaftaran/form_control/cek_disertasi'); ?>",
						type	: "POST",            
						data: "id="+id,
...
function simpan_disertasi()
	{
					...
					$.ajax(
					{
						url 	: "<?php echo base_url('pendaftaran/form_control/insert_disertasi'); ?>",
						type	: "POST",            
						data: $("#data_disertasi").serialize(),
						success: function(x)
						{
							
							$('#slide-form').load("<?php echo base_url('pendaftaran/form_control/data_disertasi/"+id+"'); ?>");
...
function hapus_disertasi()
	{
			$.ajax(
					{
						url 	: "<?php echo base_url('pendaftaran/form_control/hapus_disertasi'); ?>",
						type	: "POST",            
						data    : "id="+id,
						success: function(z)
						{
