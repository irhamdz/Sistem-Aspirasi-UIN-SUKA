<?php 
echo "<input type='hidden' id='user' value='".$this->session->userdata('username')."'>";
...
{		
		nf++;
		var x=document.getElementById('crumbs').innerHTML;
		x+="<li id='"+nf+"'><a href=''>"+(data_form[i].replace('_',' ')).toUpperCase()+"</a></li>";
		document.getElementById('crumbs').innerHTML=x;

}

function kurangi_posisi()
{
...
var jalur=$('#jalur').attr('value');
	function selanjutnya () 
	{
		...
		{ 
				...
				$.ajax(
				{
					url 	: "<?php echo base_url('pendaftaran/daftar_mhs_c/"+data_form[position]+"'); ?>",
					type	: "POST",            
					...
					success: function(r)
					{
						...
						{ 
							...
							$('#slide-form').load("<?php echo base_url('pendaftaran/form_control/"+data_form[position]+"/"+user+"'); ?>");
							}
							else
							{ 
							$('#slide-form').load("<?php echo base_url('pendaftaran/form_control/"+data_form[position]+"/"+user+"/"+jalur+"'); ?>");
